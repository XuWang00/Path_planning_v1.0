...
def vis_raycasting(index):
    ...
    ###选区功能2,通过选取三角面片顶点来框选区域，见feature_selection_workflow.py
...
########################################################################################################################
    ...
    for idx, viewpoint in enumerate(filtered_viewpoints):
        ...
        hits_intersection = filter_triangles_by_depth_optimized(eye_center, center, objmesh, hits_intersection, min_depth=settings['min_depth'], max_depth=settings['max_depth'])
...
##TODO: this is still not usable now, if you want to transfer the coordinates, you still need to use coo_transformer.py and my setup in RoboDK. May use ROS replace this later.
# def path_plan():
#     settings = initialize_settings()  # Load settings from config file
#     while True:
...
if __name__ == "__main__":
    ...
    while True:
        command = input("Enter 'ray' to run ray-casting visualization for pre-check, 'itr' to run Optimal-viewpoints generation, 'pose' to run viewpoints-result visualization, "
                        "'path' to run path planning, or 'exit' to quit: ").strip().lower()
        if command == 'pose':
            #Here you need to type in the Optimal-viewpoints result, which you want to check
            vis_pose(viewpoint_indices=[82, 460, 475, 343, 415, 396, 408, 573, 282, 559, 542, 419, 366, 548, 252, 375, 260, 239])
        elif command == 'ray':
            vis_raycasting(index=10)
    ##TODO: Should combine vis_raycasting() and iteration() in one workflow
        elif command == 'itr':
    ##TODO: The selection function in vis_raycasting workflow is still not connected to this workflow. already done test in iteration_plus_selection_workflow.py
            iteration()
        # elif command == 'path':  #not usable for now
        #     path_plan()
        elif command == 'exit':
            print("Exiting the program.")
            break
        else:
            print("Invalid command. Please enter 'vis', 'itr', 'pos' or 'exit'.")
